"""ANJ (Autorité Nationale des Jeux) — French quarterly and annual gambling market data."""
...
class ANJFranceScraper(RegulatorScraper):
    name = "ANJ"
    market_slug = "france"
    base_url = (
        "https://anj.fr/les-chiffres-du-marche-regule"
    )
    ...
    def scrape(self) -> list[ScrapedMetric]:
        ...
        with http_client() as client:
            try:
                ...
                self.log.warning("ANJ: index fetch failed (%s)", exc)
                return records
            ...
            for year, month, url, link_text in candidates:
                ...
                try:
                    ...
                    self.log.warning("ANJ %04d-%02d: %s", year, month, exc)
                    continue
    ...
    def _parse(self, text: str, year: int, month: int, url: str,
        ...
        def add(code: str, patterns: list[str]) -> None:
            ...
            recs.append(ScrapedMetric(
                ...
                notes=f"ANJ Q{quarter} {year} (link: {link_text[:80]})",
                source_url=url,
            ))

        add("ggr", [r"produit\s+brut\s+des\s+jeux", r"pbj", r"total\s+ggr"])
        add("sportsbook_handle", [r"mises\s+sur\s+les\s+paris\s+sportifs", r"total\s+handle"])
        add("sportsbook_revenue", [r"paris\s+sportifs"])
        return recs
