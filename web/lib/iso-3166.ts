// ISO 3166-1 numeric → alpha-2 lookup. Used by the Overview world map
// because the standard world-atlas TopoJSON (countries-110m.json) keys
// each Geography by ISO numeric (`geo.id` is a 3-digit string like
// "076" for Brazil), but our markets table uses ISO alpha-2 codes
// ("BR"). This static table is ~5KB minified and avoids pulling in
// i18n-iso-countries (~600KB) just for the lookup.
...
// Helpful inverse for callers that have alpha-2 and need to find the
// numeric id (rarely needed since topojson is keyed by numeric).
